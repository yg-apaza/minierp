...
public class CuentaDao
{
    ...
    {
        Query query = session.createQuery("from Cuenta C where C.estRegCod = 'A' and C.cueNiv = :niv");
        query.setParameter("niv", 1);
        List<Cuenta> cuentas = query.list();
        List<CuentaDto> nuevos = new ArrayList<CuentaDto>();
        System.out.println("SIZEEEE: " + cuentas.size());
        for(int i = 0; i < cuentas.size(); i++){
            try {
                ...
                nuevos.add(nuevo);
                System.out.println("agregaaaar");
            } catch (IllegalAccessException | InvocationTargetException ex) {
                System.out.println("okaaaaaaaaaaaaaay");
                return null;
            }
        }
    ...
    {
        Query query = session.createQuery("from Cuenta C where C.estRegCod = 'A' and C.cuePad = :pad");
